...
// I don't know how long we'll keep this loop but this will be like the main place we keep users while in the app... The app will be a loop basically.
    loop {
        ...
        match input.trim() {
            ...
            cmd if cmd.starts_with('r') && cmd.len() > 1 => {
                if let Ok(block_num) = cmd[1..].parse::<usize>() {
                    if block_num > 0 && block_num <= cached_blocks.len() {
                        let code = &cached_blocks[block_num - 1];

                        // Ensure the code has a main function
                        ...
                        // The show_execution_result function already handles this
                    } else {
                        warn_count += 1;
                    }
                } else {
            ...
            "r" => {
                // Just 'r' without number - show help
                println!("\n{} Press r1, r2, etc. to run specific code blocks", "[hint]".yellow().bold());
